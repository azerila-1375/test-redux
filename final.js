...
(function (global) {
  const { configureStore } = global.RTK;

  const store = configureStore({
    reducer: {
      counter: global.counterSlice.reducer,
      auth: global.authSlice.reducer,
    },
  });

  global.store = store;
  global.actions = {
    counter: global.counterSlice.actions,
    auth: global.authSlice.actions,
  };
})(globalThis);
